...
 Overload the == operator ( two books will be deemed equal if they have
the same ISBN)
 Overload the != operator
 Overload the &lt;&lt; operator to display title, author , ISBN and genre on
separate lines.
2. A Class Patron that contains:
 User name: string
 Card number :string
 Owed fees: integer
...
3. Create a Transaction class that contains:
 A Book
 A Patron
 Activity ( check out or check in a book)
 A Date
4. A class Library that contains:
 A vector of Books
 A vector of Patrons
 A vector of Transactions
 A function to add a book to the library
 A function to check out a Book : Whenever a user checks out a book,
have the library make sure that both the user and the book are in the
library. If they aren’t, report an error. Then check to make sure that the

user owes no fees. If the user does, report an error. If not, create a
Transaction, and place it in the vector of Transactions. Also write a
